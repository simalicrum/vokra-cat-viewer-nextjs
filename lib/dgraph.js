...
export const deleteCatLocation = (InternalIDs) => {
  let query = gql`
    mutation UpdateLocation($InternalIDs: [Cat]) {
      updateLocation(
        input: { filter: { Tier2: null }, remove: { Cats: $InternalIDs } }
      ) {
        numUids
      }
    }
...
export const deleteCatAdoptionFeeGroup = (InternalIDs) => {
  let query = gql`
    mutation UpdateAdoptionFeeGroup($InternalIDs: [Cat]) {
...
export const deleteCatAttributes = (InternalIDs) => {
  let query = gql`
    mutation UpdateAttribute($InternalIDs: [Cat]) {
      updateAttribute(
