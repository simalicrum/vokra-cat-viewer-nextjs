...
export const createCats = (cats) => {
  let requests = gql`
    mutation CreateCats($cats: [CatInput]) {
...
export const updateCats = (cats) => {
  let requests = gql`
    mutation UpdateCats($cats: [CatInput]) {
      updateCats(cats: $cats) {
        InternalID
      }
    }
  `;
...
export const deleteAttributesByCats = (IDs) => {
  let requests = gql`mutation DeleteAttributesByCat {`;
  for (let ID of IDs) {
    requests = requests.concat(
      gql`
          delete${ID}:deleteAttributesByCat(ID: "${ID}") {
            InternalID
          }
      `
    );
  }
  requests = requests.concat(gql`}`);
  return graphQLClient.request(requests);
};
